Feature: Forge settings documentation stays synchronized with defaults
  ...
  The Forge settings defaults live in the validated config model. Every
  documented settings example must stay identical to those defaults so readers
  never copy stale configuration.

  Scenario: forge settings sample is generated from the validated defaults
    Given the validated Forge config model defines the default settings
